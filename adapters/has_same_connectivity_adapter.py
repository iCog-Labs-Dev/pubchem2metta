...
class HasSameConnectivityAsEdge(Adapter):
    ...
    HAS_SAME_CONNECTIVITY_AS = rdflib.term.URIRef("http://semanticscience.org/resource/CHEMINF_000462")
    ...
    def get_edges(self):
        self.graph = self.__get_graph(file_path=self.file_path)
        self.cache_edge_properties()
        
        edges = list(self.graph.subject_objects(predicate=self.HAS_SAME_CONNECTIVITY_AS, unique=True))
    ...
    def __get_graph(self, file_path):
    ...
    def cache_edge_properties(self):
        ...
        self.cache['on_property'] = self.cache_predicate(HasSameConnectivityAsEdge.ON_PROPERTY)
        self.cache['some_values_from'] = self.cache_predicate(HasSameConnectivityAsEdge.SOME_VALUES_FROM)
        self.cache['all_values_from'] = self.cache_predicate(HasSameConnectivityAsEdge.ALL_VALUES_FROM)
